...
def admin_delete_topic(topic_id):
    ...
    cur = conn.cursor()
    try:
        # Проверяем проекты с этой темой
        cur.execute("SELECT COUNT(*) FROM projects WHERE topic_id = %s", (topic_id,))
        count = cur.fetchone()['count']

        # Если есть проекты, убираем у них тему
        if count > 0:
            cur.execute("UPDATE projects SET topic_id = NULL WHERE topic_id = %s", (topic_id,))
        ...
        if count > 0:
            flash(f'Тема удалена. {count} проектов остались без темы.', 'warning')
        else:
            flash('Тема удалена.', 'success')

        log_action(session['user_id'], 'delete_topic', f'Удалена тема {topic_id}')
